...
* 就地操作，使用指标标记子序列；统一使用左闭右开表示法；
* 参考值取子序列第一个元素，两个指针`left, right`分别指向列表的第一个元素和最后一个元素，使用`arr[left]`和`key`，`arr[right]`和`key`进行比较，不断交换`arr[left], arr[right]`，直至完成一次遍历（即`right<left`），